#include "AnalysisMethods/TnPUtils/interface/ZGeneratorLineShape.h"
...
ZGeneratorLineShape::ZGeneratorLineShape(const char *name, const char *title,
  ...
  dataHist(0)
{
  TString genfileloc = TString::Format("%s/src/%s",getenv("CMSSW_BASE"),genfile);
  TFile *f_gen= TFile::Open(genfileloc);
