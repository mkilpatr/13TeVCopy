...
public:
  ...
  void run(){
    ...
    eventPlots("passPre__"           , passPreselction);
    ...
    eventPlots.fill(prodQL             , weight, "quark_likeli"       , "ln[ #prod L_{q} ]"                        , 25, -24 ,  1  , prefix);
...
void processT2bWSearchRegions(string fname = "evttree.root", string outputPath = "plots.root", string treeName = "TestAnalyzer/Events", bool isMCTree = true) {

  Analyze a(fname, treeName, isMCTree);
  a.prefix = gSystem->BaseName(fname.data()) ;
  a.prefix += "_";
