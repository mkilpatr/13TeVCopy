...
void plotTTbarCR(const TString conffile="plotttbarcr_2e.conf",
                 const TString inputdir="trees_ttbarcr_100615",
                 const TString outputdir="plots_ttbarcr/100615/2e",
                 const TString channel="2e",
                 const double  sigscale = -1,
                 const bool    plotlog = false,
                 const TString lumistr = "0.209",
                 const TString format = "png")
{
  ...
  if(plotlog)
    plots->setLogy();
  plots->setWgtVar(lumistr+"*weight");
  plots->setHeaderText("#sqrt{s} = 13 TeV","209 pb^{-1}",channel);
  ...
  sel["filt"]          = " && passcscflt && passeebadscflt && passhbheflt";
  ...
  if(channel == "2l") {
    ...
    plots->addTreeVar("metnohf_trigall",                         "metnohf",                      sel["trigall"] + sel["jets"],                                                     "No HF #slash{E}_{T} [GeV]", 25, 0, 500);
