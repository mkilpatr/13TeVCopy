...
const TString local_wgtvar = wgtvar;
...
void MakeQCDRespTailSF(){
  ...
  local_METPresel = "met >= 250 && (origmet / calomet < 5)" + trigSR + datasel;
...
//Set binning
  selRecoBins.push_back(TString::Format("pseudoRespCSV <  %.3f", defaults::CSV_LOOSE));  selRecoBinNames.push_back("Jet is not light b-tagged"); selRecoMathNames.push_back("Jet is not \\bl");
  selRecoBins.push_back(TString::Format("pseudoRespCSV >= %.3f", defaults::CSV_MEDIUM)); selRecoBinNames.push_back("Jet is medium b-tagged");    selRecoMathNames.push_back("Jet is \\bm");
  selGenBins.push_back("trueRespFlv != 4");                                              selGenBinNames.push_back("!b");                         selGenMathNames.push_back("\\text{Non-}\\bjets");
  selGenBins.push_back("trueRespFlv == 4");                                              selGenBinNames.push_back("b");                          selGenMathNames.push_back("\\bjets");
//  selRecoBins.push_back("pseudoRespPseudoGenPT < 900"); selRecoBinNames.push_back("p_{T, pseudo} < 900"); selRecoMathNames.push_back("\\ptpseudo < 900");
//  selRecoBins.push_back("pseudoRespPseudoGenPT > 900"); selRecoBinNames.push_back("p_{T, pseudo} > 900"); selRecoMathNames.push_back("\\ptpseudo > 900");
//  selGenBins.push_back("trueRespGenPT < 900");          selGenBinNames.push_back("p_{T, true} < 900");    selGenMathNames.push_back("\\pttrue < 900");
//  selGenBins.push_back("trueRespGenPT > 900");          selGenBinNames.push_back("p_{T, true} > 900");    selGenMathNames.push_back("\\pttrue > 900");
...
//  region_bins[1].push_back("0.33");
  ...
  genSelMathNames.push_back(make_pair(selGenMathNames[0], "$\\rjet > " + region_bins[0][0] + "$"));
  for(int iB = 0; iB < region_bins[0].size() - 1; ++iB){
    ...
    genSelMathNames.push_back(make_pair(selGenMathNames[0], "$" + region_bins[0][iB + 1] + " < \\rjet < " + region_bins[0][iB] + "$"));
  }
  ...
  genSelMathNames.push_back(make_pair(selGenMathNames[0], "$" + region_bins[0][region_bins[0].size() - 1] + " > \\rjet$"));
  ...
  genSelMathNames.push_back(make_pair(selGenMathNames[1], "$\\rjet > " + region_bins[1][0] + "$"));
  for(int iB = 0; iB < region_bins[1].size() - 1; ++iB){
    ...
    genSelMathNames.push_back(make_pair(selGenMathNames[1], "$" + region_bins[1][iB + 1] + " < \\rjet < " + region_bins[1][iB] + "$"));
  }
  ...
  genSelMathNames.push_back(make_pair(selGenMathNames[1], "$" + region_bins[1][region_bins[1].size() - 1] + " > \\rjet$"));

  for(int iB = 0; iB < recoSels.size(); ++iB){
    cout << recoSels[iB] << endl;
  }
  for(int iB = 0; iB < genSels.size(); ++iB){
    cout << genSels[iB] << ", " << genSelNames[iB] << ", " << genSelMathNames[iB].first << ", " << genSelMathNames[iB].second << endl;
  }
...
//  tree_qcd  = getTree(inputDir + "/qcd_orig_tree.root");
...
cout << local_wgtvar << "*(" << ttbarWNormSels[iS] << ")" << endl;
    TH1F* dataH  = normHisto.getHistogram(tree_data, ttbarWNormSels[iS], "1.0",         TString::Format("Data_%i",  iS));
    TH1F* qcdH   = normHisto.getHistogram(tree_qcd,  ttbarWNormSels[iS],  local_wgtvar, TString::Format("qcd_%i",   iS));
    TH1F* otherH = normHisto.getHistogram(tree_bkg,  ttbarWNormSels[iS],  local_wgtvar, TString::Format("other_%i", iS));
    otherH->Add(qcdH);
    dataH->Divide(otherH);
...
cout << sel << endl;
  ...
  SFGetter::PseudoVectorGausGenerator dataVector(tree_data, tree_bkg, &bkgSFs, &bkgUNCs, sel, "1.0", local_wgtvar, recoSels, rand);
  cout << "Loading MC!" << endl;
  SFGetter::PseudoMatrixBootstrapGenerator mcMatrix(tree_qcd, sel, local_wgtvar, recoSels, genSels, 50, "bootstrapWeight");
  ...
  cout << "Loading data!" << endl;
  SFGetter::PseudoVectorGausGenerator dataVectorNoBKGUnc(tree_data, tree_bkg, &bkgSFs, 0, sel, "1.0", local_wgtvar, recoSels, rand);
  mcMatrix.reset();
...
//    allUnc.push_back(TMath::Sqrt(SFTotStatUnc[i] *SFTotStatUnc[i]));
  ...
  tabFile << "    \\resizebox*{\\textwidth}{!}{" << endl;
  tabFile << "      \\begin{tabular}{|c|"; for(unsigned int i = 0; i < nDim; ++i) tabFile << "c|"; tabFile << "}" << endl;
  tabFile << "        \\hline" << endl;
  tabFile << "         & \\multicolumn{" << region_bins[0].size() + 1 << "}{c|}{$" << genSelMathNames[0].first << "$} & \\multicolumn{" << region_bins[1].size() + 1 << "}{c|}{$" << genSelMathNames[region_bins[0].size() + 1].first << "$} \\\\ \\hline" << endl;
  tabFile << "         & ";                    for(unsigned int i = 0; i < nDim; ++i){ TString tempString = genSelMathNames[i].second                                                                                                                     + (i == nDim - 1 ? "" : " & "); tabFile << tempString; } tabFile << " \\\\ \\hline" << endl;
...
//THESE CUTS ARE FOR hm_nb1_highmtb_nj7
...
//                                                                      make_pair("respFilt",    make_pair("pseudoRespPassFilter == 1",                                 "passRespPassFilter")) };
//THESE CUTS ARE FOR lm_nb2_lowmtb_highptisr_medptb12
                                                                      make_pair("nvtau0",    make_pair("nvetotau==0",               "N_{vtau} = 0")),
                                                                      make_pair("lmNoDPhi",  make_pair(cutMap.at("lmNoDPhi" ),      rootCutMap.at("lmNoDPhi" ))),
                                                                      make_pair("nb2",       make_pair(cutMap.at("nb2"      ),      rootCutMap.at("nb2"      ))),
                                                                      make_pair("lowmtb",    make_pair(cutMap.at("lowmtb"   ),      rootCutMap.at("lowmtb"   ))),
                                                                      make_pair("highptisr", make_pair(cutMap.at("highptisr"),      rootCutMap.at("highptisr"))),
                                                                      make_pair("medptb12",  make_pair(cutMap.at("medptb12" ),      rootCutMap.at("medptb12" ))),
                                                                      make_pair("respFilt",  make_pair("pseudoRespPassFilter == 1", "passRespPassFilter")) };
...
////THESE CUTS ARE FOR hm_nb1_highmtb_nj7
////  cutDefsNames.push_back(make_pair("respFilt", make_pair("pseudoRespPassFilter == 1", "passRespPassFilter")));
...
//  cutDefsNames.push_back(make_pair("respFilt",  make_pair("pseudoRespPassFilter == 1", "passRespPassFilter")));
//  cutDefsNames.push_back(make_pair("nvtau0",    make_pair("nvetotau==0",               "N_{vtau} = 0")));
//  cutDefsNames.push_back(make_pair("lmNoDPhi",  make_pair(cutMap.at("lmNoDPhi" ),      rootCutMap.at("lmNoDPhi" ))));
//  cutDefsNames.push_back(make_pair("nb2",       make_pair(cutMap.at("nb2"      ),      rootCutMap.at("nb2"      ))));
//  cutDefsNames.push_back(make_pair("lowmtb",    make_pair(cutMap.at("lowmtb"   ),      rootCutMap.at("lowmtb"   ))));
//  cutDefsNames.push_back(make_pair("highptisr", make_pair(cutMap.at("highptisr"),      rootCutMap.at("highptisr"))));
//  cutDefsNames.push_back(make_pair("medptb12",  make_pair(cutMap.at("medptb12" ),      rootCutMap.at("medptb12" ))));
...
//  }
  ...
  vector <int> colors = { 821, 811, 812, 813 };
  ...
  cout << sel << endl;
  cout << local_wgtvar << "*(" << sel + " && " + selGenBins[((iB + 1) % 2)] << ")" << endl;
      TH1F* dataH  = histG.getHistogram(tree_data, sel,                                       "1.0",        TString::Format("Data_%i",  iB));
      TH1F* otherH = histG.getHistogram(tree_bkg,  sel,                                       local_wgtvar, TString::Format("other_%i", iB));
      TH1F* qcdH   = histG.getHistogram(tree_qcd,  sel + " && " + selGenBins[((iB + 1) % 2)], local_wgtvar, TString::Format("qcd_%i",   iB));
  ...
  cout << local_wgtvar << "*(" << sel + " && " + genSels[iSB + (iB == 0 ? 0 : region_bins[iB - 1].size() + 1)] << ")" << endl;
        qcdH = histG.getHistogram(tree_qcd, sel + " && " + genSels[iSB + (iB == 0 ? 0 : region_bins[iB - 1].size() + 1)], local_wgtvar, TString::Format("qcd_%u_%u", iB, iSB));
