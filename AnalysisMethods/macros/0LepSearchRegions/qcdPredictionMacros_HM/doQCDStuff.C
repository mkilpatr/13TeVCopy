...
//      } else if(names[iT] == "Non-QCD bkg"){
//        hist_nonQCD_bkg = dPhi_HG->getHistogram(trees[iT], sel, QCDSupport::stdMCWeightNonQCD, TString::Format("nonqcd_%s", name.Data()));
...
//      } else if(names[iT].Contains("T2tt") || names[iT].Contains("T2fbd")){
...
void MakeMetJetPlots(QCDSupport::SRegInfo& srinfo, vector<TString>& names, vector<TTree*>& trees, vector<int>& colors) {
  ...
  HistogramGetter* dPhi_HG = new HistogramGetter("metj", "metj123", "min((j_{1,2,3} + #slash{#it{E}}_{T}))", 100, 0, 1000);
  for(unsigned int iSR = 0; iSR < srinfo.nSR; ++iSR){
    TString title = TString::Format("%s", srinfo.srSelNames[iSR].Data());
...
//    plot_allMet->setLogy();
    ...
    plot_allMet->setTitle(" ");
    plot_allMet->setLegend(.6,.58,.90,.88);
    plot_allMet->addLine(0.18, 0.0, 0.18, yMax, 616, 2, "#Delta#phi* < 0.18");
    c->cd();
    plot_allMet->draw(c,false,QCDSupport::format);
    c->cd();
    c->Modified();
...
////  srinfo.fillTFVariations(QCDSupport::inputDir + "/qcd_smeared_CHEF_tree_skimmed_baseline.root",
  ...
  trees.emplace_back(QCDSupport::getTree(QCDSupport::inputDir + "/nonQCD_tree.root"));                             names.push_back("Lep Electron");       colors.push_back(kGreen + 2);
  trees.emplace_back(QCDSupport::getTree(QCDSupport::inputDir + "/nonQCD_tree.root"));                             names.push_back("Lep Muon");           colors.push_back(kRed);
  trees.emplace_back(QCDSupport::getTree(QCDSupport::inputDir + "/nonQCD_tree.root"));                             names.push_back("Lep Tau");            colors.push_back(kBlue);
