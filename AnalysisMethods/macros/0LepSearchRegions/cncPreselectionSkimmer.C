...
public:
  ...
  bool jetNoLep;
  Copier(string fileName, string treeName, string outFileName, bool isMCTree, ConfigPars * pars)
        : TreeCopierAllBranches(fileName,treeName,outFileName,isMCTree,pars) {
	  TFile* recoFile = TFile::Open("/uscms/home/mullin/nobackup/stuff2015/quarkGluonTagger/QGDisc_reco_ak4_731_fullPtEta.root", "READ");
  ...
  double getRecoMVA(const RecoJetF& jet, const JetReader * reader){
	double pt  = jet.pt();
	double eta = TMath::Abs(jet.eta());
	if (eta>=2.4 and pt>=158 ) pt=128;
    const Panvariate* recoMVA = paramRec->get(pt,eta);
    if(!recoMVA) return -1;
    ...
    recoMVA->setVariable( reco_ak4_jet_ptD     ,reader->jetptD_  ->at(jet.index()) );
    recoMVA->setVariable( reco_ak4_jet_axis1   ,reader->jetaxis1_->at(jet.index()) );
    recoMVA->setVariable( reco_ak4_jet_axis2   ,reader->jetaxis2_->at(jet.index()) );
    recoMVA->setVariable( reco_ak4_jet_jetMult ,reader->jetMult_ ->at(jet.index()) );
  ...
  virtual bool fillEvent() {
    ...
    // >= 1 Tight b-tag     <- removing for first set of skims
    //int ntb_ak4 = 0;
    //int ntb_pky = 0;
    //for(auto* j : bJets)      if(isTightBJet(*j)) ntb_ak4++;
    //for(auto* j : pickyBJets) if(isTightBJet(*j)) ntb_pky++;
    //if(ntb_pky + ntb_ak4 == 0) return false;

    // >= 5 jets            <- loosen to >=3 jets for first set of skims
    unsigned int minJets = 3;
    if(nJets < minJets && pickyJets.size() < minJets) return false;

    // zero leptons
    if(nVetoedLeptons>0) return false;

    // dphi(MET, jet[1,2])>0.5, dphi(MET,j3)>0.3
...
void cncPreselectionSkimmer( string processName
                           , string outFileTag
                           , double crossSection
                           , string inFileName
                           , double lumi       =  4
                           , double nEvents    = -1
                           , string treeName   = "TestAnalyzer/Events"
                           , string outPostfix = "skimmed") {
  string outFileName = "skimmed_" + outFileTag + ".root";
  string inFilePath  = "/eos/uscms/store/user/vdutta/13TeV/040215/merged/" + inFileName;
  BaseTreeAnalyzer::ConfigPars pars;
  //pars.cleanJetsvSelectedLeptons_ = false; // <- false is the default
  //pars.leptonSelection = BaseTreeAnalyzer::SEL_0_LEP;
  pars.defaultJetCollection = BaseTreeAnalyzer::AK4JETS;

  //R// get the output name
  //R// TString prefix(inFileName);
  //R// prefix.Remove(0,prefix.Last('/') + 1);
  //R// if(prefix.First('.') >= 0) prefix.Resize(prefix.First('.'));
  //R// TString outName(outFileName);

  //get the process
  defaults::Process process = defaults::NUMPROCESSES;
  for(unsigned int iP = 0; defaults::PROCESS_NAMES[iP][0]; ++iP)
	  if(defaults::PROCESS_NAMES[iP] == processName)
		  process = static_cast<defaults::Process>(iP);
  ...
  //R// Copier a(inFileName,treeName,outName.Data(),process != defaults::DATA, &pars);
  Copier a(inFilePath,treeName,outFileName,process!=defaults::DATA,&pars);
  //set weight and process
  a.weight = lumi*crossSection*1000. / ( nEvents>0 ? nEvents : float(a.getEntries())  );
  a.process = process;

  clog <<  "Skimming "       << a.getEntries()
	   << " events of type " << defaults::PROCESS_NAMES[a.process]
       << " and weight "     << a.weight
       << " into file "      << outFileName
       << endl;
  a.analyze();
}
