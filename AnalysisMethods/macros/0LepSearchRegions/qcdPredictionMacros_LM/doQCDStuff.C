...
void MakeCRPlots(QCDSupport::CRegInfo& crinfo, vector<TString>& names, vector<TTree*>& trees, vector<int>& colors) {
  ...
  TString title = "integrated_met_regions";
  TString name = "getRegion_2_to_9";
  Plot * plot_Int = new Plot(name,title,"Regions","Events");
  TH1F * data_Int = new TH1F("data_Int", "data_Int", 8, 0.5, 8.5);
  TH1F *  qcd_Int = new TH1F( "qcd_Int",  "qcd_Int", 8, 0.5, 8.5);
  TH1F * Oqcd_Int = new TH1F("Oqcd_Int", "Oqcd_Int", 8, 0.5, 8.5);
  TH1F * Nqcd_Int = new TH1F("Nqcd_Int", "Nqcd_Int", 8, 0.5, 8.5);
  TH1F * sig1_Int = new TH1F("sig1_Int", "sig1_Int", 8, 0.5, 8.5);
  TH1F * sig2_Int = new TH1F("sig2_Int", "sig2_Int", 8, 0.5, 8.5);
  TH1F * sig3_Int = new TH1F("sig3_Int", "sig3_Int", 8, 0.5, 8.5);
  TH1F *  unc_Int = new TH1F( "unc_Int",  "unc_Int", 8, 0.5, 8.5);
  float TOTyMax = 0.;
    ...
    for(unsigned int iT = 0; iT < trees.size(); ++iT){
      if(names[iT] == "Data"){
        ...
        if(QCDSupport::integrating_over_met && (iS != QCDSupport::CRegInfo::CR_nb0_hgh_boost_low_nj) && (iS != QCDSupport::CRegInfo::CR_nb0_hgh_boost_hgh_nj) ){
          ...
          data_Int->SetBinError  (iS - 1, tempHist->GetBinError  (1));
...
//        TH1F * hSN = crinfo.CRmetGetters[iS]->getHistogram(trees[iT],qcdWVSEL,QCDSupport::stdMCWeight,TString::Format("tree_%u",iT));
...
//      }
        ...
        if(QCDSupport::integrating_over_met && (iS != QCDSupport::CRegInfo::CR_nb0_hgh_boost_low_nj) && (iS != QCDSupport::CRegInfo::CR_nb0_hgh_boost_hgh_nj) ){
          ...
          if(names[iT].Contains("425")){
            ...
            sig1_Int->SetBinError  (iS - 1, tempHist->GetBinError  (1));
          } else if(names[iT].Contains("850")){
            sig2_Int->SetBinContent(iS - 1, tempHist->GetBinContent(1));
            sig2_Int->SetBinError  (iS - 1, tempHist->GetBinError  (1));
          } else if(names[iT].Contains("500")){
            sig3_Int->SetBinContent(iS - 1, tempHist->GetBinContent(1));
            sig3_Int->SetBinError  (iS - 1, tempHist->GetBinError  (1));
          }
...
//    plot->getLegend()->SetNColumns(2);
  ...
  if(QCDSupport::integrating_over_met){
    for(unsigned int iT = 0; iT < trees.size(); ++iT){
      if(names[iT] == "Data"){
        plot_Int->addHist(data_Int, names[iT], "E0", colors[iT], 0, colors[iT]);
      }
//      else if(names[iT] == "Without #it{r}_{jet} corr"){
//      }
      else if(names[iT] == "With orig. QCD MC"){
        plot_Int->addHistForRatio(Oqcd_Int, names[iT], "hist", colors[iT], 0, colors[iT]);
      }
      else if (names[iT] == "Non-QCD bkg"){
        plot_Int->addToStack(Nqcd_Int, names[iT], colors[iT], 1001, 1, 1, 3);
      }
      else if (names[iT].Contains("T2tt")){
        if(names[iT].Contains("425")){
          plot_Int->addHist(sig1_Int, names[iT], "hist", colors[iT], 0, colors[iT]);
        } else if(names[iT].Contains("850")){
          plot_Int->addHist(sig2_Int, names[iT], "hist", colors[iT], 0, colors[iT]);
        } else if(names[iT].Contains("500")){
          plot_Int->addHist(sig3_Int, names[iT], "hist", colors[iT], 0, colors[iT]);
        }
      }
      else {
        plot_Int->addToStack(qcd_Int, names[iT], colors[iT], 1001,1,1,3);
      }
    }

    plot_Int->setUncertaintyHist((TH1F*)(unc_Int));
    plot_Int->setPlotRatioUncertaintyBand();

    TCanvas * c = new TCanvas;
    plot_Int->setDrawCMSLumi();
    plot_Int->setUsePoisson();
    plot_Int->setYRange(0.1, 1000. * TOTyMax);
    plot_Int->setTitle(" ");
    plot_Int->setLegend(.65,.58,.95,.88);
  //    plot_Int->getLegend()->SetNColumns(2);

    plot_Int->setLogy();
    c->cd();
    plot_Int->drawRatioStack(c,false,QCDSupport::format);
    c->cd();
    c->Modified();
    TLatex *tl = new TLatex();
    tl->SetTextSize(0.03);
    tl->DrawLatexNDC(0.2, 0.7, "Integrated #slash{#it{E}}_{T} Regions");

    QCDSupport::setTitleOffset(c);
    c->Update();
    c->SaveAs(outputDir + "/QCD_CR_2_to_9." + QCDSupport::format);
  }
}
...
void MakeMetJetPlots(QCDSupport::SRegInfo& srinfo, vector<TString>& names, vector<TTree*>& trees, vector<int>& colors) {
  cout << "MakeMetJetPlots..." << endl;
  const TString outputDir = "MakeMetJetPlots";
  gSystem->mkdir(outputDir, true);
  
  HistogramGetter* dPhi_HG = new HistogramGetter("metj123", "metj123", "min((j_{1,2,3} + #slash{#it{E}}_{T}))", 100, 0, 1000);
  for(unsigned int iSR = 0; iSR < srinfo.nSR; ++iSR){
    ...
    TString name  = TString::Format("met_jet_%s", srinfo.srRegBinNames[iSR].Data());
    TString intName = TString::Format("met_jet_%s_Integral", srinfo.srRegBinNames[iSR].Data());
    TString sel   = TString::Format("%s && %s && %s && (nvetolep==0) && (nvetotau==0)", QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
cout << title << endl;
cout << name  << endl;
cout << sel   << endl;
    Plot* plot_allMet = new Plot(name, title, dPhi_HG->plotInfo->xTitle, "Events");
    Plot* plot_int = new Plot(intName, intTitle, dPhi_HG->plotInfo->xTitle, "Events");
    float yMin = 99999., yMax = 0., yMinInt = 99999., yMaxInt = 0.;
    float totMCYield = 0;
...
//    plot_allMet->setLogy();
    ...
    plot_int->setYRange(0, 1.4 * yMaxInt);
    plot_int->setTitle(" ");
...
void MakeDPhiStarPlots(QCDSupport::SRegInfo& srinfo, vector<TString>& names, vector<TTree*>& trees, vector<int>& colors) {
  cout << "MakeDPhiStarPlots..." << endl;
  const TString outputDir = "MakeDPhiStarPlots_MetBinning";
  gSystem->mkdir(outputDir, true);
  
  HistogramGetter* dPhi_HG = new HistogramGetter("dphistar", "min(dphistarmetj1, min(dphistarmetj2, dphistarmetj3))", "min(#Delta#phi*(j_{1,2,3},#slash{#it{E}}_{T}))", 32, 0, 3.2);
  for(unsigned int iSR = 0; iSR < srinfo.nSR; ++iSR){
    TString title = TString::Format("%s", srinfo.srSelNames[iSR].Data());
    TString intTitle = TString::Format("%s_Integral", srinfo.srSelNames[iSR].Data());
    TString name  = TString::Format("dphistar_%s", srinfo.srRegBinNames[iSR].Data());
    TString intName = TString::Format("dphistar_%s_Integral", srinfo.srRegBinNames[iSR].Data());
    TString sel   = TString::Format("%s && %s && %s && (nvetolep==0) && (nvetotau==0)", QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
cout << title << endl;
cout << name  << endl;
cout << sel   << endl;
...
//    plot_allMet->setLogy();
    ...
    plot_allMet->setTitle(" ");
    plot_allMet->setLegend(.6,.58,.90,.88);
    plot_allMet->addLine(0.35, 0.0, 0.35, yMax, 616, 2, "#Delta#phi* < 0.35");
    plot_allMet->addLine(0.1, 0.0, 0.1, yMax, 632, 2, "#Delta#phi* < 0.1");
    c->cd();
    plot_allMet->draw(c,false,QCDSupport::format);
    c->cd();
    c->Modified();
    ...
    c->Update();
    c->SaveAs(outputDir + "/" + plot_int->getName() + TString(".") + QCDSupport::format);
//    for(int iMet = 0; iMet < srinfo.nMETBins[iSR]; ++iMet){
//      if(iMet != srinfo.nMETBins[iSR] - 1){
//        title = TString::Format("%.f #leq #slash{#it{E}}_{T} < %.f, %s", srinfo.metBins[iSR][iMet], srinfo.metBins[iSR][iMet + 1], srinfo.srSelNames[iSR].Data());
//        name  = TString::Format("dphistar_%s_met_%.f_to_%.f", srinfo.srRegBinNames[iSR].Data(), srinfo.metBins[iSR][iMet], srinfo.metBins[iSR][iMet + 1]);
//        sel   = TString::Format("met >= %.f && met < %.f && %s && %s && %s && (nvetolep==0) && (nvetotau==0)", srinfo.metBins[iSR][iMet], srinfo.metBins[iSR][iMet + 1], QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
//      } else {
//        title = TString::Format("%.f #leq #slash{#it{E}}_{T}, %s", srinfo.metBins[iSR][iMet], srinfo.srSelNames[iSR].Data());
//        name  = TString::Format("dphistar_%s_met_ge_%.f", srinfo.srRegBinNames[iSR].Data(), srinfo.metBins[iSR][iMet]);
//        sel   = TString::Format("met >= %.f && %s && %s && %s && (nvetolep==0) && (nvetotau==0)", srinfo.metBins[iSR][iMet], QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
//      }
//cout << title << endl;
//cout << name  << endl;
//cout << sel   << endl;
...
void MakeDPhiPlots(QCDSupport::SRegInfo& srinfo, vector<TString>& names, vector<TTree*>& trees, vector<int>& colors) {
  cout << "MakeDPhiPlots..." << endl;
  const TString outputDir = "MakeDPhiPlots_MetBinning";
  gSystem->mkdir(outputDir, true);
  ...
  for(unsigned int iSR = 0; iSR < srinfo.nSR; ++iSR){
    TString title = TString::Format("%s", srinfo.srSelNames[iSR].Data());
    TString intTitle = TString::Format("%s_integral", srinfo.srSelNames[iSR].Data());
    TString name  = TString::Format("dphi_%s", srinfo.srRegBinNames[iSR].Data());
    TString intName = TString::Format("dphi_%s_integral", srinfo.srRegBinNames[iSR].Data());
    TString sel   = TString::Format("%s && %s && %s && (nvetolep==0) && (nvetotau==0)", QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
cout << title << endl;
cout << name  << endl;
cout << sel   << endl;
...
//    plot_allMet->setLogy();
    ...
    plot_allMet->setTitle(" ");
    plot_allMet->setLegend(.6,.58,.90,.88);
    plot_allMet->addLine(0.5, 0.0, 0.5, yMax, 1, 2, "#Delta#phi > 0.5");
    c->cd();
    plot_allMet->draw(c,false,QCDSupport::format);
    c->cd();
    c->Modified();
...
//      if(iMet != srinfo.nMETBins[iSR] - 1){
//        title = TString::Format("%.f #leq #slash{#it{E}}_{T} < %.f, %s", srinfo.metBins[iSR][iMet], srinfo.metBins[iSR][iMet + 1], srinfo.srSelNames[iSR].Data());
//        name  = TString::Format("dphi_%s_met_%.f_to_%.f", srinfo.srRegBinNames[iSR].Data(), srinfo.metBins[iSR][iMet], srinfo.metBins[iSR][iMet + 1]);
//        sel   = TString::Format("met >= %.f && met < %.f && %s && %s && %s && (nvetolep==0) && (nvetotau==0)", srinfo.metBins[iSR][iMet], srinfo.metBins[iSR][iMet + 1], QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
//      } else {
//        title = TString::Format("%.f #leq #slash{#it{E}}_{T}, %s", srinfo.metBins[iSR][iMet], srinfo.srSelNames[iSR].Data());
//        name  = TString::Format("dphi_%s_met_ge_%.f", srinfo.srRegBinNames[iSR].Data(), srinfo.metBins[iSR][iMet]);
//        sel   = TString::Format("met >= %.f && %s && %s && %s && (nvetolep==0) && (nvetotau==0)", srinfo.metBins[iSR][iMet], QCDSupport::METPresel.Data(), QCDSupport::BaselineExtraCuts.Data(), srinfo.srSel[iSR].Data());
//      }
//cout << title << endl;
//cout << name  << endl;
...
//  srinfo.fillTFVariations(QCDSupport::inputDir + "/qcd_smeared_CHEF_tree_skimmed_baseline.root",
  ...
  trees.emplace_back(QCDSupport::getTree(QCDSupport::inputDir + "/QCD_smeared_tree.root"));                names.push_back("Smeared QCD MC");    colors.push_back(color_qcd);
  ...
  //MakeDPhiPlots(        srinfo, names, trees, colors);
  //MakeDPhiStarPlots(    srinfo, names, trees, colors);
  MakeMetJetPlots(      srinfo, names, trees, colors);
