...
const TString outputDir = "validate_smearing_MetBinning";
...
void ValidateQCDSmearing(){
  ...
  stackTrees.emplace_back(QCDSupport::getTree(QCDSupport::inputDir + "/QCD_smeared_tree.root"),                   "Smeared");           colors.push_back(color_qcd);
  stackTrees.emplace_back(QCDSupport::getTree(QCDSupport::inputDir + "/../plots_16_11_15_NoSmear/qcd_orig_tree.root"),    "With orig. QCD MC"); colors.push_back(color_tW);
  ...
  vector <pair <TString, pair <TString, TString> > > MetJet = {                     make_pair("METPresel_&&_BaselineExtraCuts_signal",  make_pair("metj123 > 0", ""))};
...
//  pair <TString, TString> met_Info = make_pair("met", "#slash{#it{E}}_{T}");
  ...
  HistogramGetter* MetJet_shape  = new HistogramGetter("metj123",     "min(metj123, 1000000)",                                   "min(#slash{#it{E}}_{T} + j_{123})",             400,   0., 1000.);
...
//  printStack(stackTrees, ht_shape,    "stack", nb_binning, met2_binning, QCDSupport::CR_def, "Events", true);
  ...
  printStack(stackTrees, MetJet_shape,           "stack", MetJet,                  QCDSupport::METPresel + " && " + QCDSupport::BaselineExtraCuts, "Events",false,false,true);
