...
// Procedure: Need a default set of trees + one set for each systematic variation up/down. The directory structure should be like /blah/whatever/trees, /blah/whatever/trees/btagup, /blah/whatever/trees/btagdown, /blah/whatever/trees/jesup, /blah/whatever/trees/jesdown, etc., with a set of up/down for each entry in the "vars" vector
//

void getSysUncertainties(TString sname = "T2tb_300_100",
                         ...
                         const TString json = "")
{
  ...
  vector<TString> vars = {"btag","btagfsim","jes","pu","lep"};
  ...
  TFile* outfile = 0;
  if(dolepcr) outfile = new TFile(outputdir + "/" + sname + "_onelepcr_sysvars.root","RECREATE");
  else        outfile = new TFile(outputdir + "/" + sname + "_sysvars.root","RECREATE");

  FILE * txtfile = 0;
  if(dolepcr) txtfile = fopen(outputdir + "/" + sname + "_onelepcr_sysuncs.txt","w");
  else        txtfile = fopen(outputdir + "/" + sname + "_sysuncs.txt","w");
  ...
  for(auto var : vars) {
    TString varupfilename = fullname;
    varupfilename.ReplaceAll(sname, "/"+var+"up/"+sname);
    TFile* infileup = TFile::Open(varupfilename, "READ");
    assert(infileup);
    TTree* intreeup = (TTree*)infileup->Get("Events");
    assert(intreeup);

    TString vardownfilename = fullname;
    vardownfilename.ReplaceAll(sname, "/"+var+"down/"+sname);
    TFile* infiledown = TFile::Open(vardownfilename, "READ");
    assert(infiledown);
    TTree* intreedown = (TTree*)infiledown->Get("Events");
    assert(intreedown);

    int isrbin = 0;
    for(auto srbin : srbins) {
      TString hname = "nominal_"+srbin+"_"+sname;
      TString hnameup = var+"up_"+srbin+"_"+sname;
      TString hnamedown = var+"down_"+srbin+"_"+sname;
      ...
      intreeup->Project(hnameup,"met",basecutstr);
      Plot::addOverFlow(hvarup);

      intreedown->Project(hnamedown,"met",basecutstr);
      ...
      for(int ibin = 1; ibin < h->GetNbinsX()+1; ++ibin) {
        double varup = hvarup->GetBinContent(ibin)-h->GetBinContent(ibin);
        double vardown = hvardown->GetBinContent(ibin)-h->GetBinContent(ibin);
        printf("bin_%s%d_%s\t%5.3f\t%5.3f\t(%5.3f)\n", dolepcr ? "onelepcr_" : "", int(h->GetBinLowEdge(ibin)), binlabels[isrbin].Data(), varup, vardown, h->GetBinContent(ibin));

        double varavg = 0.5*(varup - vardown);

        printf("bin_%s%d_%s\t%sunc\t%s\t%4.3f\n", dolepcr ? "onelepcr_" : "", int(h->GetBinLowEdge(ibin)), binlabels[isrbin].Data(), var.Data(), sname.Data(), (1.0 + (varavg/h->GetBinContent(ibin))));
        fprintf(txtfile, "bin_%s%d_%s\t%sunc\t%s\t%4.3f\n", dolepcr ? "onelepcr_" : "", int(h->GetBinLowEdge(ibin)), binlabels[isrbin].Data(), var.Data(), sname.Data(), (1.0 + (varavg/h->GetBinContent(ibin))));
      }
