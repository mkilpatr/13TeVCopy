...
void plotLepCR(){
  ...
  z.plotDataMC({"ttbar", "wjets", "tW", "ttW", "qcd"}, "singlelep", false);
...
//  vector<TString> vars = {"csvj1pt_1"};
...

  {
    ...
    for (auto &norm : normMap){
      ...
      for (auto var : vars){
        hists["noaddback_"+norm.first].push_back(z.plotDataMC(varDict.at(var), {"ttbar", "wjets", "tW", "ttW", "qcd"}, "singlelep", Category::dummy_category(), false));
      }
    }
...

  {
    ...
    for (auto &norm : normMap){
      ...
      for (auto var : vars){
        hists["addback_"+norm.first].push_back(z.plotDataMC(varDict.at(var), {"ttbar", "wjets", "tW", "ttW", "qcd"}, "singlelep", Category::dummy_category(), false));
      }
    }
