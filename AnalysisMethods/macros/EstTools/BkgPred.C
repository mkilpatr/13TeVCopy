...
void BkgPred(){
  ...
  vector<TH1*> pred;
  pred.push_back(convertToHist(q.yields.at("_pred"),"qcd_pred",";Signal Region;Events"));
  pred.push_back(convertToHist(l.yields.at("_pred"),"ttbarplusw_pred",";Signal Region;Events"));
  pred.push_back(convertToHist(z.yields.at("_pred"),"znunu_pred",";Signal Region;Events"));

  vector<TH1*> mc;
  mc.push_back(convertToHist(q.yields.at("qcd-sr-withveto"),"qcd_mc",";Signal Region;Events"));
  mc.push_back(convertToHist(l.yields.at("ttbarplusw-sr"),"ttbarplusw_mc",";Signal Region;Events"));
  mc.push_back(convertToHist(z.yields.at("znunu"),"znunu_mc",";Signal Region;Events"));

  TH1* hmctotal = nullptr;
  for (auto *h : mc){
    if (h==mc.front())
      hmctotal = (TH1*)h->Clone("mctotal");
    else
      hmctotal->Add(h);
  }
  ...
  s.calcYields();
  vector<TH1*> sig;
  sig.push_back(convertToHist(s.yields.at("T2fbd_375_295"),"T2fbd_375_295",";Signal Region;Events"));
  sig.push_back(convertToHist(s.yields.at("T2fbd_375_325"),"T2fbd_375_325",";Signal Region;Events"));
  sig.push_back(convertToHist(s.yields.at("T2fbd_375_355"),"T2fbd_375_355",";Signal Region;Events"));
  ...
  vector<TString> datalabel = {"MC (shown as data)"};
  vector<Color_t> sigcolors = {kViolet-1, kBlue, kMagenta};
  for (auto *h : sig) h->SetLineStyle(kDashed);

  {
    // plot pred and sig
    prepHists(pred, false, false, true);
    prepHists({hmctotal}, false, false, false, {kBlack});
...
//    prepHists(sig, false, false);
    ...
    appendLegends(leg, {hmctotal}, datalabel, "LP");
//    appendLegends(leg, sig, siglabels);
    leg->SetTextSize(0.03);
//    leg->SetNColumns(2);
    leg->SetY1NDC(leg->GetY2NDC() - 0.2);
    auto c = drawStackAndRatio(pred, hmctotal, leg, "N_{MC}/N_{exp}", 0.001, 2.999);
    c->Print(s.config.outputdir+"/pred.pdf");
    c->Print(s.config.outputdir+"/pred.C");
  }
...
//    leg->SetNColumns(2);
    ...
    auto c = drawStack(mc, sig, true, leg);
  ...
  TFile *output = new TFile(sigcfg.outputdir+"/pred.root", "RECREATE");
