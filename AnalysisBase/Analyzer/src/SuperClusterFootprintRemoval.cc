...
//
SuperClusterFootprintRemoval::SuperClusterFootprintRemoval(edm::Handle<pat::ElectronCollection>& electronsHandle,edm::Handle<pat::MuonCollection>& muonsHandle,edm::Handle<pat::PhotonCollection>& photonsHandle,edm::Handle<pat::JetCollection>& jetsHandle, edm::Handle<pat::PackedCandidateCollection>& pfCandidate, CaloSubdetectorGeometry *barrelgeometry, CaloSubdetectorGeometry *endcapgeometry, MagneticField *magfield,const double photonptMinRC_, const double jetptMinRC_,const double leptonptMinRC_,edm::ParameterSet iConfig)
{
  ...
  barrelGeometry=barrelgeometry;
  endcapGeometry=endcapgeometry;
  magField=magfield;

  electronHandle=electronsHandle;
  muonHandle=muonsHandle;
...
sc_xtal_information SuperClusterFootprintRemoval::GetSCXtalInfo(reco::SuperClusterRef sc){
  ...
  for (unsigned int i=0;i<sc->clustersSize(); i++){
    const std::vector< std::pair<DetId, float> > & seedrechits = sc->clusters()[i]->hitsAndFractions();
...
bool SuperClusterFootprintRemoval::FindCloseJetsAndPhotons(float rotation_phi){
  ...
  //if found is true cone is vetoed
  return found;
