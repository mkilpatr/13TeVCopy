  ...
  const BTagEntry::Parameters &par) const
{
  ...
  if (!data_.count(tok)) {
std::cerr << "ERROR in BTagCalibration (b-tagging efficiency correction code): "
...
#ifdef DEBUG
//  std::cout << "total entries found for this configuration: " << tmpData_.size() << std::endl;
//  std::cout << "trying to read flavor for this config: " << jf << std::endl;
#endif
  ...
  // HACK: couldn't locate entry with pt in its range. loop again, assume they're pt ordered, and return first or last.
  // first entry for extreme low pt
  const BTagCalibrationReader::TmpEntry &e_first = entries.at(0);
  if(pt < e_first.ptMin) {
#ifdef DEBUG
      std::cout << "hack: returned low end of first entry" << std::endl;
      std::cout << "low entry has pts: " << e_first.ptMin << " " << e_first.ptMax << std::endl;
      std::cout << "zero for this entry gives: " << e_first.func.Eval(0) << std::endl;
#endif
      return e_first.func.Eval(e_first.ptMin);
  }
  // last entry for excessive pt
  const BTagCalibrationReader::TmpEntry &e_last = entries.at(entries.size()-1);
  if(pt >= e_last.ptMax) {
#ifdef DEBUG
    std::cout << "hack: returned high end of last entry" << std::endl;
#endif
    return e_last.func.Eval(e_last.ptMax);
  }
...
#ifdef DEBUG
    std::cout << "Properties of found entry: " << std::endl;
    std::cout << "    Flavor:  " << be.params.jetFlavor << std::endl;
    std::cout << "    Operating point: " << be.params.operatingPoint << std::endl;
    std::cout << "    Measurement type: " << be.params.measurementType << std::endl;
    std::cout << "    Systematics type: " << be.params.sysType << std::endl;
    std::cout << "    Flavor:  " << be.params.jetFlavor << std::endl;
    std::cout << "    Eta range: " << be.params.etaMin << " " << be.params.etaMax << std::endl;
    std::cout << "    Pt range" << be.params.ptMin << " " << be.params.ptMax << std::endl;
    std::cout << "    Formula: " << be.formula.c_str() << std::endl;
    std::cout << std::endl; 
#endif
