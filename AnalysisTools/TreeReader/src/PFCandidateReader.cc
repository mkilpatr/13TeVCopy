...
void PFCandidateReader::load(TreeReader *treeReader, int options, string branchName)
{
  ...
  if(options_ & LOADRECO){
    ...
    if(options_ & LOADEXTRECO){
      ...
      treeReader->setBranchAddress(branchName , "contJetDR"    , &contjetdr   , true);
      treeReader->setBranchAddress(branchName , "contJetCSV"   , &contjetcsv  , true);
    }
  }
  if(options_ & FILLOBJ)
    clog << "+Objects";
...
void PFCandidateReader::refresh(){
  ...
  if(options_ & LOADEXTRECO){
    ...
    for(unsigned int iL = 0; iL < pt->size(); ++iL){
      ...
      cand.setContJetDR(contjetdr->at(iL));
      cand.setContJetCSV(contjetcsv->at(iL));
