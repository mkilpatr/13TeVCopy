...
//_____________________________________________________________________________
bool ParticleInfo::findBosonDaughters(const std::vector<reco::GenParticle> &genParticles, const reco::GenParticle* &boson, const reco::GenParticle* &dau1, const reco::GenParticle* &dau2)
{
...
//_____________________________________________________________________________
void ParticleInfo::findTauDaughter(const std::vector<reco::GenParticle> &genParticles, const reco::GenParticle* &tau, const reco::GenParticle* &dau)
{
  for(const reco::GenParticle &p : genParticles) {
    const reco::Candidate* mom = p.mother(0);
    if(mom==nullptr) continue;             // not filled
    ...
    if(isA(p_tauminus,&p) || isA(p_nu_tau,&p) || isA(p_Wplus, &p) || isA(p_gamma,&p)) continue;    // get the 1st lepton/hadron from the tau decay, status 2 should imply a decayed hadron in this case
    if(!dau) {
      dau = &p;                            // fill daughter
      break;
    }
...
//_____________________________________________________________________________
std::ostream& ParticleInfo::printGenHistory(const std::vector<reco::GenParticle>& genParticles, const unsigned int particleIndex)
{
  ...
  if      (particle.numberOfMothers() == 1)
    ...
    std::cout  << "...";
  std::cout    << " -> " << particleIndex << ":" << titleFor(particle.pdgId(),particle.charge());
  return std::cout;
}

//_____________________________________________________________________________
void ParticleInfo::printGenInfo(const std::vector<reco::GenParticle>& genParticles, int genBound)
{
  using namespace std;
  ...
  for (int iGen = 0; iGen < genBound; ++iGen) {
    ...
    cout  << (genParticle.status() == status_doc ? "|" : " ") << (isOutgoing(&genParticle) ? "* " : "  ");
    cout  << right << setw(3) << iGen << " " << setw(10) << genParticle.pdgId() << "  ";
    cout  << left << setw(20) << titleFor(genParticle.pdgId(),genParticle.charge());
